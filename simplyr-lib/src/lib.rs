...
pub fn pay_as_bid_matching(input: &MarketInput) -> MarketOutput {
	...
	// Sort by price
	bids.sort_by(|a, b| a.price_euro_per_kwh.total_cmp(&b.price_euro_per_kwh).reverse());
...
pub fn custom_fair_matching(
	...
	loop {
		...
		fair_bids.sort_by(|a, b| a.price_euro_per_kwh.total_cmp(&b.price_euro_per_kwh).reverse());
		// Sort by adjusted price, ascending, then by price, descending
		fair_asks.sort_by(|a, b| {
			a.adjusted_price
				.total_cmp(&b.adjusted_price)
				.then(a.price_euro_per_kwh.total_cmp(&b.price_euro_per_kwh).reverse())
		});
...
mod tests {
	...
	fn test_pay_as_bid() {
...

		{
			...
			let market_input = MarketInput { orders: vec![order_1, order_2, order_3] };
